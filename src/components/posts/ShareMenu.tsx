...
export function ShareMenu({ title, slug, size = "md", label = true }: ShareMenuProps) {
  ...
  const iconSize = size === "sm" ? "w-3.5 h-3.5" : "w-[17px] h-[17px]";
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Share2 className={iconSize} strokeWidth={1.9} />
        {label && (copied ? "¡Copiado!" : "Compartir")}
      </button>

      {open && (
        <div
          className="absolute z-50 mt-2 w-48 rounded-xl border border-zinc-200 bg-white shadow-lg py-1.5 animate-in fade-in slide-in-from-top-1 duration-150"
          style={{ right: 0 }}
        >
          {items.map((item) => (
            <a
